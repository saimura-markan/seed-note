...
export default function CorrectionSubmit() {
  ...
  const handleSubmit = async () => {
    ...
    const nextStatus = complaint?.status === 'report_rejected'
      ? 'supervisor_check'
      : ['是正案承認', 'correction_rejected'].includes(complaint?.status)
      ? '改善報告書提出'
      : '是正案提出'
    await supabase.from('complaints').update({ status: nextStatus, current_turn_started_at: new Date().toISOString() }).eq('id', id)
    setSubmitting(false)
