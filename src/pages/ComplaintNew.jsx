...
export default function ComplaintNew() {
  ...
  const receivedStr = receivedAt.toLocaleString('ja-JP', {
  ...
  return (
    <div className="min-h-screen px-6 py-6" style={{ backgroundColor: '#F5F0E8' }}>
      <div className="max-w-2xl mx-auto pb-28">
        ...
        <form id="complaint-form" onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-5 gap-2">
              {EMOTION_LEVELS.map(({ level }) => (
                <button key={level} type="button"
                  ...
                  className={cn(
                    'flex items-center justify-center py-1.5 rounded-xl border-2 text-sm font-bold transition-all',
                    form.emotionLevel === level
                      ? 'text-white'
                      : 'bg-stone-100 border-stone-100 text-gray-500 hover:border-stone-300'
                  )}
                  style={form.emotionLevel === level
